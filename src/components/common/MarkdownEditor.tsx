...
function blocksToStr(value: Block[]) {
  return JSON.stringify(compress(value));
}

/** Try JSON parsing first, fallback to Markdown parsing if it fails or is invalid. */
async function strToBlocks(value: string, editor: BlockNoteEditor) {
  try {
    if (value) {
      let blocks;
      try {
        blocks = JSON.parse(value);
        blocks = decompress(blocks);
